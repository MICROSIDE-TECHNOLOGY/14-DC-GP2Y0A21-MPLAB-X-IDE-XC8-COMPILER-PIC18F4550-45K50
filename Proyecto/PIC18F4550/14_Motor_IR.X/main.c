...
void main( void )
{
    ADCInit();                               // Configura el módulo ADC en el canal 0
    InitPorts();                             // Configura el puerto A0 como entrada analógica y C2 como salida digiral
    PWMinit();                               // Configura el módulo PWM
    while ( 1 ) {
        uint16_t PWM_Set = ReadADC() * 1.53; // Escala el valor analógico a el ciclo PWM
        if ( PWM_Set < 1024 )
            PWM2_LoadDutyValue( PWM_Set );   // Genera la señal PWM
    }
    return;
}
